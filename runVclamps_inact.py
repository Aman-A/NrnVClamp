...
# -*- coding: utf-8 -*-
"""
Created on Sat May  2 15:38:56 2020
...
clamp_params_inact = {'amp1':-120, 
                    ...
                    'v_stepf':-10,
                    'dv_step': 5,
                    'amp_to_step': 'amp1', # activation protocol steps 2nd phase
                    'rs':1e-3,# MOhm
                    'x':0.5}
clamp_params = [clamp_params_inact]
# NEURON settings
# Settings
dt = 0.001 # 1 µs
Ex = {'ena': 50,'ek':-100} # 55 for Kole 2008
T = 37
...
for chan_name,_ in channels.items():
    ...
    clamp_test = Vclamp_tester(chan_name=chan_name,curr_name=curr_name,gbar_name=channels[chan_name]['gbar'],\
                                   g_name=channels[chan_name]['g'],clamp_params=clamp_params,dt=dt,T=T,q10=q10,**Ex)
    # Run vclamp simulations
    for params in clamp_params:
        v_steps,t_vec,vs,currs,gs = clamp_test.run_protocol(params)
        cpeak_global = np.nanmax([np.nanmax(np.abs(c)) for c in currs])    

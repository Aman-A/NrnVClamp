...
UNITSOFF
		...
		minf = (1/(1+exp(-1*(Vm+4-minfshift)/7)))^(1/3)
		hinf = 1/(1+exp((Vm+28-hinfshift)/4.56))
		sinf = 1/(1+exp((Vm+50)/7.5))
		tau_m = 0.03 + .5/(exp((Vm-mtaushift)/12)+exp(-1*(Vm+29-mtaushift)/18)) 
		:tau_h = 2.3+7.9/(exp((Vm+28)/17.6)+exp(-1*(Vm+54)/23.3))
		tau_h = 2.63+250/(exp((Vm+36-htaushift)/7.7)+exp(-1*(Vm-htaushift)/16.3)) + 1.4/(1+exp(-1*(Vm+0.6)/2.95))
